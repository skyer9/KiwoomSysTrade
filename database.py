...
DATABASES = create_engine('mysql+mysqldb://root:abcd1234@localhost:3306/db_stock?charset=utf8', echo=False)
...
class StockDayCandleChart(Base):
    ...
    종목코드 = Column(String(6), primary_key=True)
    일자 = Column(String(10), primary_key=True)
    현재가 = Column(Numeric(10,2), nullable=False)
    거래량 = Column(Numeric(10, 2), nullable=False)
    거래대금 = Column(Numeric(10, 2), nullable=False)
    시가 = Column(Numeric(10, 2), nullable=False)
    고가 = Column(Numeric(10, 2), nullable=False)
    저가 = Column(Numeric(10, 2), nullable=False)
    전일종가 = Column(Numeric(10, 2), nullable=False)
    regdate = Column(DateTime)
    lastupdate = Column(DateTime)

    def __init__(self, 종목코드, 현재가, 거래량, 거래대금, 일자, 시가, 고가, 저가, 전일종가):
        self.종목코드 = 종목코드
        self.현재가 = 현재가
        self.거래량 = 거래량
        self.거래대금 = 거래대금
    ...
    def __repr__(self):
        return "<StockBasicInfo('%s', '%s')>" % (self.stock_code, self.stock_name)
